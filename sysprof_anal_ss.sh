...
if [ $sysprof_version = "1.1" ]; then
	...
	readarray -t megafile_array <<< "$(cat $file_init| grep oraclemain |awk -F '|' '{print $1"|"$7"|"$3}'|grep -v "||")"
	readarray -t megafile_array_local <<< "$(cat $file_init| grep -v oraclemain| awk -F '|' '{print $1"|"$6"|"$3}'|grep -v "||" | grep -v localhost)"
else
	printf "\nRunning for Sysprof version: ${bold}${lime_yellow}1.3${normal}\n";
	readarray -t megafile_array <<< "$(cat $file_init| grep oraclemain |awk -F '|' '{print $1"|"$10"|"$6}'|grep -v "||")"
	readarray -t megafile_array_local <<< "$(cat $file_init| grep -v oraclemain| awk -F '|' '{print $1"|"$9"|"$6}'|grep -v "||" | grep -v localhost)"
fi;

# Array with full timeline from 0000 to 2355:
	unset array_full    
	array_full=(`printf '%s\n' "${megafile_array[@]}" | awk -F '|' '{ print $1; }' | rev | cut -c 1-4 | rev |uniq`)
...
function gen_stat_get {  #If arguments for unittype ($2 aka $exact_unit) is passed.
	print_header
	unset array_units; 
	# Defining if we have run function for localhos processes only. If $3 then we have to run localhost processes only, else external clients.
	if [ ! -z "$3" ]; then  my_array="${megafile_array_local[@]}"; else my_array="${megafile_array[@]}" ; fi
	...
	for units in $1; do
		array_units=( $(printf '%s\n' $my_array | grep "$units" | awk -F '|' '{print $3,$2}' | sort -n | awk '!seen[$0]++' | tail -10 -r |awk '{print $2}'| awk '!seen[$0]++' | head -1) );
			for every_unit in "${array_units[@]}"; do
				if [ ! -z "$every_unit" ]; then 																										#awk ‘{print $(NF)}’
					...
					echo "$(printf '%s\n' $my_array | grep $units |grep $filedate$every_hour|grep $every_unit | eval $2)"; done`;
				fi;
			done;
	done
	print_bench
...
function exact_unit_stat_get {  #If arguments for unittype ($2 aka $exact_unit) was not passed.
	print_header
	unset list_to_check;
	list_to_check=( $( printf '%s\n' "${megafile_array[@]}" | grep $exact_unit | awk -F '|' '{print "|"$2"|"}' | sort -n | awk '!seen[$0]++'| tr '\n' ' ') );
		for every_exact_unit in "${list_to_check[@]}"; do
				if [ ! -z "$exact_unit" ]; then 
					printf "%-9s|%-5s|%-5s|%-5s|%-5s|%-5s|%-5s|%-5s|%-5s|%-5s|%-5s|%-5s|%-5s|%-5s|%-5s|%-5s|%-5s|%-5s|%-5s|%-5s|%-5s|%-5s|%-5s|%-5s|%-5s\n" `echo "$every_exact_unit"| tr -d '|'` `for every_hour in "${array_hours[@]}"; do 
					echo "$(printf '%s\n' ${megafile_array[@]} | grep "$every_exact_unit" |grep $filedate$every_hour | eval $1)"; done`;
				fi;
		done;
	print_bench
}
...
if [ ! -n "$2" ]; then  #If arguments for unittype ($2 aka $exact_unit) not passed.
	...
	printf "\n${bold}${lime_yellow}Summ of RAM consumption by URE sessions from single SDP Client(in Megabytes). For top RAM consumenrs only.${normal}\n";
